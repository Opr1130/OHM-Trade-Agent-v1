from app.scanner.models import MarketSnapshot, ScoreCard


def score_snapshot(snapshot: MarketSnapshot) -> ScoreCard:
    score = 0
    strengths: list[str] = []
    weaknesses: list[str] = []
    warnings: list[str] = []

    if snapshot.last_price > snapshot.ema20:
        score += 10
        strengths.append("Price above EMA20")
    else:
        weaknesses.append("Price below EMA20")

    if snapshot.ema20 > snapshot.ema50:
        score += 15
        strengths.append("EMA20 above EMA50")
    else:
        weaknesses.append("EMA20 not above EMA50")

    if snapshot.ema50 > snapshot.ema200:
        score += 15
        strengths.append("EMA50 above EMA200")
    else:
        weaknesses.append("EMA50 not above EMA200")

    if 45 <= snapshot.rsi <= 65:
        score += 15
        strengths.append("RSI in healthy momentum range")
    elif 35 <= snapshot.rsi < 45:
        score += 8
        warnings.append("RSI weak but not oversold")
    elif 65 < snapshot.rsi <= 72:
        score += 8
        warnings.append("RSI strong but approaching overbought")
    elif snapshot.rsi < 35:
        weaknesses.append("RSI weak or oversold")
    else:
        weaknesses.append("RSI overextended")

    if snapshot.macd_line > snapshot.macd_signal:
        score += 5
        strengths.append("MACD bullish confirmation")
    else:
        weaknesses.append("MACD not confirmed")

    if snapshot.volume_ratio >= 1.5:
        score += 20
        strengths.append("Strong volume expansion")
    elif snapshot.volume_ratio >= 1.1:
        score += 14
        strengths.append("Volume above average")
    elif snapshot.volume_ratio >= 0.8:
        score += 7
        warnings.append("Volume near average")
    else:
        weaknesses.append("Volume below average")

    if 0.5 <= snapshot.atr_pct <= 3.0:
        score += 10
        strengths.append("ATR in tradable range")
    elif 0.25 <= snapshot.atr_pct < 0.5:
        score += 5
        warnings.append("ATR slightly low")
    else:
        weaknesses.append("ATR outside preferred range")

    if snapshot.trend == "bullish":
        score += 10
        strengths.append("Bullish trend")
    elif snapshot.trend == "neutral":
        score += 4
        warnings.append("Neutral trend")
    else:
        weaknesses.append("Bearish trend")

    return ScoreCard(
        score=min(score, 100),
        strengths=strengths,
        weaknesses=weaknesses,
        warnings=warnings,
    )
