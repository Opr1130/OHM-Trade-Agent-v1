...
SELECT
    ...
    count(*) FILTER (
        WHERE coalesce((payload->>'outcome_complete')::boolean, false)
          AND coalesce((payload->>'market_winner')::boolean, false)
    ) AS market_winner_candidates,
    count(*) FILTER (
        WHERE coalesce((payload->>'outcome_complete')::boolean, false)
          AND payload->>'opportunity_classification' = 'CAPTURED_WINNER'
    ) AS captured_winners,
    count(*) FILTER (
        WHERE coalesce((payload->>'outcome_complete')::boolean, false)
    ...
    count(*) FILTER (
        WHERE coalesce((payload->>'outcome_complete')::boolean, false)
          AND payload->>'opportunity_classification' = 'THRESHOLD_70_79_MISS_CANDIDATE'
    ) AS threshold_70_79_miss_candidates,
    count(*) FILTER (
        WHERE coalesce((payload->>'outcome_complete')::boolean, false)
          AND payload->>'opportunity_classification' = 'RANKING_OR_CAP_MISS_CANDIDATE'
    ) AS ranking_or_cap_miss_candidates,
    count(*) FILTER (
        WHERE coalesce((payload->>'outcome_complete')::boolean, false)
          AND payload->>'opportunity_classification' = 'OPERATIONAL_EXECUTABLE_MISS'
