    ...
    <div class="content">
      ...
      <div id="dashboardBody" style="display:none">
        <section class="section active" id="page-overview">
          <div class="grid">
            ...
            <div class="card span12">
              <div class="card-head"><div><div class="card-title">Early Watch → outcome funnel</div><div class="card-desc">Complete linked journeys active in the selected window. Alert delivery is shown separately because delivery is not required for qualification.</div></div></div>
        ...
        <section class="section" id="page-funnel">
          <div class="grid">
            <div class="card span12"><div class="card-head"><div><div class="card-title">Early intelligence → execution funnel</div><div class="card-desc">Sequential conversion for the Early Watch cohort. Complete journey history is retained even when the journey started before the selected window.</div></div></div><div class="funnel" id="fullFunnel"></div></div>
...
function render(){
 ...
 $('funnelInterpretation').innerHTML=`Early alert delivery coverage: <b>${pct(data.funnel.early_delivered_pct)}</b>.<br/>Early detection → qualified conversion: <b>${pct(data.funnel.early_to_signal_pct)}</b>.<br/>Paper requested → closed conversion: <b>${pct(data.funnel.requested_to_closed_pct)}</b>.<br/>Closed paper win rate: <b>${pct(data.funnel.closed_win_rate_pct)}</b>.<br/>Direct qualified journeys without Early Watch: <b>${fmt(data.funnel.direct_qualified_journeys)}</b>.<br/><br/>A falling funnel is not automatically bad: intentional rejection is valuable when rejected setups subsequently underperform. Use the Evolution and Learning Health views before changing thresholds.`;
