      ...
      <div id="dashboardBody" style="display:none">
        ...
        <section class="section" id="page-evolution">
          <div class="grid">
            ...
            <div class="card span7"><div class="card-head"><div><div class="card-title">Evolution trend</div><div class="card-desc">Daily win-rate evidence and average return. Use sample size before interpreting short-term changes.</div></div></div><div class="chart tall" id="evolutionTrend"></div></div>
        ...
        <section class="section" id="page-paper">
          <div class="grid">
            <div class="card span8"><div class="card-head"><div><div class="card-title">Realized paper P/L curve</div><div class="card-desc">USD and USDT are plotted as separate cumulative series and never added together.</div></div><div class="legend"><span>USD</span><span>USDT</span></div></div><div class="chart tall" id="equityChart"></div></div>
...
function svgLine(containerId, rows, series, ySuffix='%'){
 ...
 const W=800,H=300,L=48,R=18,T=18,B=34;
 ...
 let paths='';series.forEach((s,si)=>{let d='',started=false;clean.forEach((r,i)=>{const v=r[s.key];if(v===null||v===undefined){started=false;return}d+=(started?' L ':'M ')+x(i)+' '+y(Number(v));started=true});paths+=`<path class="${si===0?'line-primary':si===1?'line-secondary':'line-tertiary'}" d="${d}"/>`});
 ...
 el.innerHTML=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">${g}<line class="axis" x1="${L}" y1="${H-B}" x2="${W-R}" y2="${H-B}"/>${paths}${labels}</svg>`;
}
...
function render(){
 ...
 bars('funnelBars',(data.funnel.stages||[]).map(r=>({...r,segment:r.stage,avg_return_pct:r.count,samples:r.count})),'avg_return_pct');
...
function renderVisibleCharts(page){
 ...
 if(page==='evolution'){svgLine('evolutionTrend',data.trend||[],[{key:'paper_win_rate_pct'},{key:'early_watch_win_rate_pct'},{key:'avg_return_pct'}],'%')}
 if(page==='paper'){const rows=data.paper.equity_curve||[];const labels=[...new Set(rows.map(r=>r.label))];const merged=labels.map(l=>{const x={label:l};rows.filter(r=>r.label===l).forEach(r=>x[r.currency]=r.cumulative_pnl);return x});svgLine('equityChart',merged,[{key:'USD'},{key:'USDT'}],'')}
}
async function load(){
