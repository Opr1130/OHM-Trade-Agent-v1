...
<div class="wrap">
  ...
  <div id="dashboardBody" style="display:none">
    <div class="grid">
      ...
      <div class="card span4"><div class="kicker">Market</div><div class="metric"><span>Current regime</span><b id="regime">—</b></div><div class="metric"><span>Technical shortlist</span><b id="shortlist">—</b></div><div class="metric"><span>LONG / SHORT</span><b id="longShort">—</b></div><div class="metric"><span>Last scan</span><b id="lastScan">—</b></div></div>
...
function render(d){const m=d.market,a=d.ai,f=d.funnel,q=d.decisions,t=d.trading,l=d.learning,c=d.current;
 ...
 $('regime').textContent=m.latest_regime;$('shortlist').textContent=fmt(m.technical_shortlist);$('longShort').textContent=fmt(m.long_shortlist)+' / '+fmt(m.short_shortlist);$('lastScan').textContent=m.last_scan_utc?new Date(m.last_scan_utc).toLocaleString():'—';
 ...
 $('tracked').textContent=fmt(l.decisions_tracked);$('observations').textContent=fmt(l.observations);$('evaluated').textContent=fmt(l.evaluated_decisions);$('correctAvoids').textContent=fmt(l.correct_avoids);$('missed').textContent=fmt(l.missed_profitable_opportunities);
