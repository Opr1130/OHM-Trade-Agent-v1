...
def calibration_model(records: list[dict[str, Any]], *, min_samples: int = MIN_GLOBAL_SAMPLES) -> dict[str, Any]:
    ...
    for key, bucket in buckets.items():
        ...
        evidence[key] = {"samples": len(bucket), "success_rate": round(rate, 4)}
    return {"status": "CALIBRATED", "samples": len(rows), "baseline_success_rate": round(baseline, 4), "weights": weights, "evidence": evidence, "guardrail": {"max_adjustment": MAX_ADJUSTMENT, "minimum_bucket_samples": MIN_BUCKET_SAMPLES}}
