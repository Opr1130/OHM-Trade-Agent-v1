...
def detect_emergency_move(trade: ActiveTrade) -> EmergencyMoveResult:
    ...
    change_5m_pct = (
        (current - five_minutes_ago) / five_minutes_ago
    ) * 100

    change_15m_pct = (
        (current - fifteen_minutes_ago) / fifteen_minutes_ago
    ) * 100

    stop_distance_pct = (
        (current - trade.stop_price) / current
    ) * 100
    ...
    if change_5m_pct <= -2.0:
        severity = _raise_severity(severity, "critical")
        reasons.append(
            f"Rapid 5-minute drop detected ({change_5m_pct:.2f}%)"
        )
    elif change_5m_pct <= -1.0:
        severity = _raise_severity(severity, "warning")
        reasons.append(
            f"Sharp 5-minute decline detected ({change_5m_pct:.2f}%)"
        )

    if change_15m_pct <= -3.0:
        severity = _raise_severity(severity, "critical")
        reasons.append(
            f"Rapid 15-minute decline detected ({change_15m_pct:.2f}%)"
        )
    elif change_15m_pct <= -1.5:
        severity = _raise_severity(severity, "warning")
        reasons.append(
            f"15-minute weakness detected ({change_15m_pct:.2f}%)"
        )

    if 0 < stop_distance_pct <= 1.0:
        severity = _raise_severity(severity, "warning")
        reasons.append(
            f"Price is only {stop_distance_pct:.2f}% above the stop"
        )
