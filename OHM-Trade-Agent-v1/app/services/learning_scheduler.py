...
def run_learning_cycle(*, now: datetime | None = None) -> dict[str, Any]:
    """Run free/local learning work.

    Shadow observations use Kraken public ticker data only when a horizon is due.
    Strategy-profile recalibration runs at most once per 24 hours and uses only
    locally persisted OHM/Kraken evidence. No OpenAI call is made here.
    """
    now = now or _now()
