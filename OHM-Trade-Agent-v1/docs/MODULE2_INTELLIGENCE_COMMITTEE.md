...
| IC-042 | Isolated shadow deployment architecture | **IMPLEMENTED_AWAITING_DEPLOYMENT_APPROVAL** | The exact deployment change is prepared in `deploy/committee/`: a hardened `Type=oneshot` systemd unit (one bounded cycle, `OPIP_COMMITTEE_MODE=off`, resource caps, `NoNewPrivileges`, `ProtectSystem=strict`, `RestrictAddressFamilies`, `IPAddressDeny` with a provider allowlist), a disabled-by-default six-hourly timer, a worker script that refuses without an exact 40-character release SHA and records an explicit `SKIPPED_MODE_OFF` disposition, `cycle_runner.py` (one cycle, cycle budget capped from the UTC day's remainder, durable dispositions, trust report), a root-owned credential template whose every value is a placeholder, and an isolation-proof script covering every required claim. Installation goes through the owner-gated control plane `.github/workflows/deploy-committee.yml`, which mirrors `deploy-learning.yml` (owner-gated `/deploy-committee <sha>` on issue 64, target must equal current main, exact-SHA pytest required, dedicated protected `committee-shadow` environment, pinned SSH from the existing learning-host secrets) and is install-and-verify only. Nothing is deployed: the workflow has never been run. |
