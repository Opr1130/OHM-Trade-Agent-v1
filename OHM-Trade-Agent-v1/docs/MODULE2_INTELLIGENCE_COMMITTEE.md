...
| IC-042 | Isolated shadow deployment architecture | **IMPLEMENTED_AWAITING_DEPLOYMENT_APPROVAL** | The exact deployment change is prepared in `deploy/committee/`: a hardened `Type=oneshot` systemd unit (one bounded cycle, `OPIP_COMMITTEE_MODE=off`, resource caps, `NoNewPrivileges`, `ProtectSystem=strict`, `RestrictAddressFamilies`, and **deny-all egress** — `IPAddressDeny=any` with no `IPAddressAllow=` entries, so OFF-mode egress fails closed rather than depending on DNS host names), a timer that stays disabled and inactive at install, a worker script that refuses without an exact 40-character release SHA and records an explicit `SKIPPED_MODE_OFF` disposition, `cycle_runner.py` (one cycle, cycle budget capped from the UTC day's remainder, durable dispositions, trust report), a root-owned credential template whose every value is a placeholder, and `verify-committee-isolation.sh`, which proves release identity, OFF mode, deny-all egress, timer/service inertness, zero committee work, read-only evidence input, isolated advisory output, absence of trading credentials, absence of provider credentials from every systemd diagnostic surface (`systemctl show`, the unit file text, and the journal — counts and PASS/FAIL only, never a value), no inbound listener, resource limits, and a valid rollback path. Installation goes through the owner-gated control plane `.github/workflows/deploy-committee.yml`, which mirrors `deploy-learning.yml` (owner-gated `/deploy-committee <sha>` on issue 64, target must equal current main, exact-SHA pytest required, dedicated protected `committee-shadow` environment, pinned SSH from the existing learning-host secrets) and is install-and-verify only: it does not pass `--enable-timer`, and it requires successful removal of the temporary remote release directory as a deployment success condition. Nothing is deployed: the workflow has never been run. |
