...
def _notice(observation, *, provider="BINANCE"):
    bounds = WindowBounds.for_timestamp(
        asset="bitcoin",
        venue=provider,
        timestamp_utc=observation.envelope.provider_timestamp_utc,
        window_seconds=15,
    )
    return SealedWindowNotice(
        provider=provider,
        stream_type=StreamType.AGG_TRADE,
...
def test_quiet_venue_window_expires_as_incomplete_not_capacity_drop():
    ...
    accumulator.seal(_notice(first))
    assert accumulator.drain_ready() == ()

    later = _binance_trade(trade_id=2, timestamp_ms=1788048016000)
    accumulator.record(later)
    # Expiration is driven by a later finalized seal notice, not a live
    # observation, so retention-window late evidence is never cut off early.
    accumulator.seal(_notice(later))
    ...
    assert accumulator.diagnostics()["missing_venue_windows"] == 1
...
def test_malformed_feature_payload_is_rejected_without_sink_exception():
    ...
    assert accumulator.diagnostics()["invalid_payload_observations"] == 1
