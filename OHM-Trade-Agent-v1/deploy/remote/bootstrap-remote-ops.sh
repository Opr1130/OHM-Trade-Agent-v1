...
for cmd in git docker curl flock ssh-keygen ssh-keyscan install visudo; do
...
KNOWN_HOSTS="$(ssh-keyscan -p "$PORT" -H localhost 2>/dev/null || true)"
...
For OHM_DEPLOY_KNOWN_HOSTS, run from a trusted machine against the public host:
ssh-keyscan -p $PORT -H $HOST

IMPORTANT:
- Do not use the localhost keyscan printed by this bootstrap as the GitHub known-hosts secret.
